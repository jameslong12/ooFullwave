classdef fwObj < handle
    ...
    %  (created by Gianmarco Pinton). Currently only generates homogeneous
    %  field maps
    %
    %  Initialization:
    %           obj = fwObj(varargin)
    ...
    %
    %  Methods:
    %           genXdc          - Generate transducer properties within object
    %           launchSim       - Perform simulation
    %
    ...
    methods
        ...
        function obj = fwObj(varargin)
            ...
            addOptional(p,'wY',5e-2)
            addOptional(p,'wZ',5e-2)
        ...
        function obj = make_xdc(obj, name)
            if ~exist('name','var'), error('xdc name is a required field'); end
            xdc = xdc_lib(name);
            obj.xdc = xdc;
